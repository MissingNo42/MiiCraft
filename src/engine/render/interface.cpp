...
void GUI::renderInventory(Player &player) const {
	if (player.inventory.open) {
		...
		for (int i = 0; i < 27; i++) {
			...
			GX_Color1x8(WHITE);
			GX_TexCoord2f32(blocData[player.inventory.inventory[1 + player.inventory.currentPage * 3 + i / 9][i %
			                                                                                                  9].item.type].x[2],
			                blocData[player.inventory.inventory[1 + player.inventory.currentPage * 3 + i / 9][i %
			                                                                                                  9].item.type].y[2]); // Top left
		...
		for (int i = 0; i < 9; i++) {
			...
			GX_TexCoord2f32(blocData[player.inventory.inventory[0][i].item.type].x[2],
			                blocData[player.inventory.inventory[0][i].item.type].y[2]); // Top left
			
			GX_Position3f32(-0.582 + i * 0.158, -0.475, 0);
			GX_Color1x8(WHITE);
			GX_TexCoord2f32(blocData[player.inventory.inventory[0][i].item.type].x[2] + OFFSET,
			                blocData[player.inventory.inventory[0][i].item.type].y[2]); // Top right
		...
		for (int i = 0; i < 9; i++) {
			if (player.inventory.inventory[0][i].quantity > 0) {
				GX_Begin(GX_QUADS, GX_VTXFMT0, 8);
				GX_Position3f32(-0.39 + i * 0.098, -0.66, 0);
				
				GX_Color1x8(WHITE);
				GX_TexCoord2f32(BLOCK_COORD(16 + player.inventory.inventory[0][i].quantity / 10),
				                BLOCK_COORD(12)); // Top left
				
				GX_Position3f32(-0.37 + i * 0.098, -0.66, 0);
				
				GX_Color1x8(WHITE);
				GX_TexCoord2f32(BLOCK_COORD(17 + player.inventory.inventory[0][i].quantity / 10),
				                BLOCK_COORD(12)); // Top right
				
				GX_Position3f32(-0.37 + i * 0.098, -0.68, 0);
				
				GX_Color1x8(WHITE);
				GX_TexCoord2f32(BLOCK_COORD(17 + player.inventory.inventory[0][i].quantity / 10),
				                BLOCK_COORD(13)); // Bottom right
				
				GX_Position3f32(-0.39 + i * 0.098, -0.68, 0);
				...
				GX_Position3f32(-0.35 + i * 0.098, -0.66, 0);
				
				GX_Color1x8(WHITE);
				GX_TexCoord2f32(BLOCK_COORD(17 + player.inventory.inventory[0][i].quantity % 10),
				                BLOCK_COORD(12)); // Top right
				
				GX_Position3f32(-0.35 + i * 0.098, -0.68, 0);
	...
	if (player.inventory.open && player.inventory.pickedItem.item.type != BlockType::Air) {
		...
		GX_Color1x8(WHITE);
		GX_TexCoord2f32(blocData[player.inventory.pickedItem.item.type].x[2],
		                blocData[player.inventory.pickedItem.item.type].y[2]); // Top left
		
		GX_Position3f32(a - 0.01, b - 0.01, 0);
		
		GX_Color1x8(WHITE);
		GX_TexCoord2f32(blocData[player.inventory.pickedItem.item.type].x[2] + OFFSET,
		                blocData[player.inventory.pickedItem.item.type].y[2]); // Top right
		
		GX_Position3f32(a - 0.01, -y + b, 0);
		
		GX_Color1x8(WHITE);
		GX_TexCoord2f32(blocData[player.inventory.pickedItem.item.type].x[2] + OFFSET,
		                blocData[player.inventory.pickedItem.item.type].y[2] + OFFSET); // Bottom right
		
		GX_Position3f32(-x + a, -y + b, 0);
		
		GX_Color1x8(WHITE);
		GX_TexCoord2f32(blocData[player.inventory.pickedItem.item.type].x[2],
		                blocData[player.inventory.pickedItem.item.type].y[2] + OFFSET); // Bottom left
