//
// Created by Romain on 04/02/2024.
//

#include "wiimote.h"
#include <cmath>
#include "engine/render/renderer.h"

void Wiimote::setup() {
	PAD_Init();
	WPAD_Init();
	WPAD_SetDataFormat(WPAD_CHAN_ALL, WPAD_FMT_BTNS_ACC_IR);
	WPAD_SetVRes(WPAD_CHAN_ALL, Renderer::rmode->fbWidth, Renderer::rmode->xfbHeight);
}


void Wiimote::update() {
	connected = WPAD_Probe(chan, &type) == WPAD_ERR_NONE;

	wd = WPAD_Data(chan);

	x = 2 * wd->ir.x / static_cast<f32>(Renderer::rmode->fbWidth) - 1;
	y = -2 * wd->ir.y / static_cast<f32>(Renderer::rmode->xfbHeight) + 1;

	if (wd->btns_d & WPAD_BUTTON_HOME)
		quit = true;

	WPAD_Expansion(chan, &data);

	if (wd->exp.type == WPAD_EXP_NUNCHUK) {
		auto [x, y, z] = wd->exp.nunchuk.accel;

		const f32 norm = std::sqrt(static_cast<f32>(y * y + x * x + z * z));

		accel = std::abs(norm - last_accel);

		last_accel = norm;
	}
}

bool Wiimote::quit = false;

Wiimote::Wiimote(const int chan) :
	chan(chan) {}

void Wiimote::sync() {
	WPAD_ScanPads();
}
